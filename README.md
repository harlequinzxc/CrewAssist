...
- **Inflight menu printer:** Same slide-up glass overlay as the menu viewer. Numbered sector sheets (`1 LAX → NRT`). Toolbar pills match Menu sheet / Edit content size. Elegant auto-selects A4, Compact auto-selects A6 (size chips stay toggleable after). Preview is a real A4/A6 page scaled to the overlay width. Sector / Elegant-Compact / A4-A6 jump the preview to the top. Snacks except Light Bites, and drinks except Champagne / red / white wine, start hidden (Show in Edit). Menu sheet and Edit content scroll independently. Edit is a flat cabin card (not nested sheets). Export PNG / JPEG / Word. Compact is still the tight homework layout.
...
- **Architecture:** PWA (service worker `crewassist-v46`, web manifest) for install and cache. App SemVer `APP_VERSION` (currently 1.4.13) is shown on onboarding.

## File Structure
- `index.html` — UI, chat, calculators, menu overlay, printer, and all app logic.
- `manifest.json` & `sw.js` — PWA install and cache (`crewassist-v46`).
