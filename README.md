...
- **Inflight menu printer:** Same slide-up glass overlay as the menu viewer. Numbered sector sheets (`1 LAX → NRT`). Toolbar pills match Menu sheet / Edit content size. Elegant auto-selects A4, Compact auto-selects A6 (size chips stay toggleable after). Preview is a real A4/A6 page (210×297mm / 105×148mm) scaled to the overlay width so type matches print, not the browser width; A−/A+ only changes type size. Menu sheet and Edit content scroll independently. Edit is a flat cabin card (not nested sheets). Export PNG / JPEG / Word. Compact is still the tight homework layout.
...
- **Architecture:** PWA (service worker `crewassist-v45`, web manifest) for install and cache. App SemVer `APP_VERSION` (currently 1.4.12) is shown on onboarding.

## File Structure
- `index.html` — UI, chat, calculators, menu overlay, printer, and all app logic.
- `manifest.json` & `sw.js` — PWA install and cache (`crewassist-v45`).
