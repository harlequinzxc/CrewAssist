...
- **Inflight menu printer:** Same slide-up glass overlay as the menu viewer. Numbered sector sheets, Elegant or Compact layout, A4 or A6, zoom, greyscale, descriptions, in-app edit, export PNG / JPEG / Word. Elegant is the dining-card sheet: gold masthead with diamonds, navy cabin hairline band, route + vertical rule + serif service title, italic course labels, hollow rings, bold dish names, italic descriptions. Compact is still the tight homework layout. Compact is still the tight homework layout.
...
- **Architecture:** PWA (service worker `crewassist-v42`, web manifest) for install and cache. App SemVer `APP_VERSION` (currently 1.4.9) is shown on onboarding.

## File Structure
- `index.html` — UI, chat, calculators, menu overlay, printer, and all app logic.
- `manifest.json` & `sw.js` — PWA install and cache (`crewassist-v42`).
