...
- **Inflight menu printer:** Same slide-up glass overlay as the menu viewer. Switch View ↔ Print from the sheet headers (book / printer) using the current flight — no second lookup. Numbered sector sheets (`1 LAX → NRT`). Toolbar pills match Menu sheet / Edit content size. Elegant auto-selects A4, Compact auto-selects A6 (size chips stay toggleable after). Preview is a real A4/A6 page scaled to the overlay width. Sector / Elegant-Compact / A4-A6 jump the preview to the top. Snacks except Light Bites, and drinks except Champagne / red / white wine, start hidden (Show in Edit). Compact also hides bakery / hot drinks, and on SCL hides Chocolate plus Cheese and Crackers. Menu sheet and Edit content scroll independently. Edit is a flat cabin card (not nested sheets). Export PNG / JPEG / Word at high quality (compact is drawn at 300 dpi so protein invert sits on the text baseline; elegant html2canvas uses scale 3). Compact uses a per-service header with the date after the meal (`SQ478 (→JNB) JCL (SUPPER) 160926`) and a full-width rule before each service, not under the title. A Bounds pill draws the A4/A6 page edge on the preview only (never in the export). Printer loading copy is “Preparing the menu”.
...
- **Architecture:** PWA (service worker `crewassist-v55`, web manifest) for install and cache. App SemVer `APP_VERSION` (currently 1.4.22) is shown on onboarding.

## File Structure
- `index.html` — UI, chat, calculators, menu overlay, printer, and all app logic.
- `manifest.json` & `sw.js` — PWA install and cache (`crewassist-v55`).
