...
Latest app cache: `crewassist-v86` (`sw.js`). App SemVer: **1.17.0** (`APP_VERSION` in `index.html`, shown on onboarding as `#app-semver`). `APP_WHAT_NEW` is the bullet list for the What’s new **overlay**. Working branch: **this session's** `arena/<session-id>-crewassist` — resolve it, do not copy it (see **How to work in this repo → Branch**). Repo `harlequinzxc/CrewAssist`.
...
- **Allowances Archive** is specified in `LOGIC.md` §§4.9–4.12 and 5.4 / 6.8. **It is not implemented** in `index.html`. Turnaround date fields (`#…-ifa-d1` / `d2`) and layover LMA dates are ready for it. Fetch writes into those same fields.
  - **Built in v1.17.0 (owner said go, 2026-09-16), scoped to the COP summary:** `crewAssist.archive` holds entries `{ id, savedAt, monthKey, flightType, stationDisplay, amount }`; `monthKey` = first IFA sector date `YYYY-MM`, `stationDisplay` = first two LMA IATAs (Layover) or first two IFA arrival airports (Turnaround), `amount` = IFA-only for Turnaround else IFA+LMA (§4.10–4.12). A `Save to archives` button is rendered **only when `mode === 'both'`** and amount > 0, disables itself after saving. `showArchiveOverlay()` (chat intent `archive`) groups entries by month desc with a summed grand total per month, `No archived allowances yet.` when empty, delete-per-entry and Clear-all (via `appConfirm`). Deviation from §5.4 STEP 10, deliberate and reversible: saving does **not** auto-reset the calculator (surprising data loss); add it back if the owner wants. `btn-clear-data`'s `localStorage.clear()` intentionally wipes the archive too. Reset All has no distinct implementation to guard.
