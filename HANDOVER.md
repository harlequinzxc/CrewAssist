...
Latest app cache: `crewassist-v50` (`sw.js`). App SemVer: **1.4.17** (`APP_VERSION` in `index.html`, shown on onboarding). Branch work for this line of UI lives on `arena/01a0819d-crewassist`.
...
- **Step 2:** Onboarding UI, settings bottom sheet, developer mode (10 taps on the logo). Subtle SemVer `v1.4.17` at the bottom of onboarding (`#app-semver`).
...
- **Step 6 inflight menu printer:** Sheet and edit content replay `.menu-anim-in` like the overlay. Elegant drinks: Champagne / white / red on by default (not fortified); groups are spaced with hairlines. Compact hides From The Bakery and Hot Beverage courses by default. Elegant/Compact zoom and hide flags stay independent. Do not call SQ from the browser.

## In-Progress / Known Gaps

- **Printer sheet art:** Elegant dining-card is in. Compact homework layout is in progress (v1.4.17: per-service header, numbered items, protein invert, wine keys) — more compact notes still coming.
