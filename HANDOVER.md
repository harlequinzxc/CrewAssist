...
Latest app cache: `crewassist-v29` (`sw.js`). Branch work for this line of UI lives on `arena/01a0819d-crewassist`.
...
- **Step 6 inflight menu printer:** Full-screen `#print-overlay` (not a bottom sheet). Chrome: back, `SQ n · N sheets`, date/route, gold Export, numbered sector pills, Elegant/Compact, A4/A6, zoom, Greyscale, Descriptions, Menu sheet / Edit content. Elegant = restaurant-style one page (Cormorant Garamond, no cursive). Compact = tight all-caps APP/MAIN/DESSERT with protein strikethrough + beverages. Export PNG/JPEG (`html2canvas` CDN) and Word (HTML `.doc`). Edit writes back into `printState.model`. Do not call SQ from the browser.
