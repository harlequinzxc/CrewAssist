...
Latest app cache: `crewassist-v77` (`sw.js`). App SemVer: **1.9.0** (`APP_VERSION` in `index.html`, shown on onboarding as `#app-semver`). `APP_WHAT_NEW` is the bullet list for the What’s new **overlay**. Working branch: **this session's** `arena/<session-id>-crewassist` — resolve it, do not copy it (see **How to work in this repo → Branch**). Repo `harlequinzxc/CrewAssist`.

**Product status (owner, 2026-09-15):** v1.5.9 is **signed off** — its issues are fixed (LMA out-date = typed sector date; calendar weekday/Today/fixed height). **In for review: v1.5.10** (calendar hairline), **v1.6.0** (Settings → Allowance calculator interface), **v1.7.0** (modes hide calculator elements; icons, titles, ★ Recommended), **v1.8.0** (LMA labels + Shuttle position, dropdowns reset the card, Calculate gate in Default) and **v1.9.0** (Lucide icons replace all emoji, Shuttle/Direct US/Paxing are toggle switches, COP/IFA dropdowns are pill selectors). **Next (do not start until signed off):** archive, month total. Offline last menu is **on hold**. Do not hardcode SQ478/479 (or any flight) for shuttle; Fetch may *pre-tick* shuttle when ground time is same calendar day or under 6 hours (SQ11/12 NRT–LAX is a long ground, not a shuttle).
...
| `sw.js` | `crewassist-v77`. Precaches shell + `rates.json`. Network-first for navigate/document/`index.html`/`sw.js`/`rates.json`. |
