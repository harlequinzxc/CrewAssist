...
Latest app cache: `crewassist-v55` (`sw.js`). App SemVer: **1.4.22** (`APP_VERSION` in `index.html`, shown on onboarding). Branch work for this line of UI lives on `arena/01a0819d-crewassist`.
...
- **Step 2:** Onboarding UI, settings bottom sheet, developer mode (10 taps on the logo). Subtle SemVer `v1.4.22` at the bottom of onboarding (`#app-semver`).
...
- **Step 6 inflight menu printer:** Sheet and edit content replay `.menu-anim-in` like the overlay. Elegant drinks: Champagne / white / red on by default (not fortified); groups are spaced with hairlines. Compact hides From The Bakery and Hot Beverage courses by default; SCL compact also hides Chocolate (course or exact item) and Cheese and Crackers. Elegant/Compact zoom and hide flags stay independent. View ↔ print switch (`#btn-menu-to-print` / `#print-btn-view`) reuses `currentMenuSearch` (no second flight entry). Printer loading copy is “Preparing the menu”. Compact header is one line (`SQ478 (→JNB) JCL (SUPPER) 160926`). Header rule and protein invert are canvas-baked `<img>`s (`printBakeHeader` / `printBakeInvert`) so html2canvas PNG/JPEG keeps fill and type together. Bounds pill (`printState.showBounds`, default on) draws a dashed gold A4/A6 page box on the live preview; html2canvas `ignoreElements` and Word clone strip `.print-bounds` even when the pill is selected. Do not call SQ from the browser.

## In-Progress / Known Gaps

- **Printer sheet art:** Elegant dining-card is in. Compact still iterating (v1.4.21: baked header/invert for export, A4/A6 Bounds guide). More compact notes still coming.
