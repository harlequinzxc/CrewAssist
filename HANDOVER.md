...
Latest app cache: `crewassist-v44` (`sw.js`). App SemVer: **1.4.11** (`APP_VERSION` in `index.html`, shown on onboarding). Branch work for this line of UI lives on `arena/01a0819d-crewassist`.
...
- **Step 2:** Onboarding UI, settings bottom sheet, developer mode (10 taps on the logo). Subtle SemVer `v1.4.11` at the bottom of onboarding (`#app-semver`).
...
- **Step 6 inflight menu printer:** Slide-up glass sheet. Sector + toolbar pills are `h-9 text-xs` like Menu sheet / Edit content. Elegant switch selects A4, Compact selects A6; size chips remain free after. Preview paper is 100% of the overlay width, centered; A−/A+ only `--print-type`. `#print-paper-scroll` and `#print-edit-scroll` scroll independently. Edit is one card per cabin (badge, gold course rule, item X) plus Menu details. Name-only / bullet lines are bold. Export PNG/JPEG/Word. Do not call SQ from the browser.

## In-Progress / Known Gaps

- **Printer sheet art:** Elegant dining-card + printer chrome in v1.4.11. Compact homework layout still first-pass — wait for a dedicated compact reference before locking it.
