...
| `earnings-archive.test.js` | Route display strips only SIN, keeps multi-sector routes; the real current calendar month expands on open; delete uses the settings-style pop-up (OK deletes, Cancel keeps).  Tap-through: rows reopen the full summary from the stored snapshot (Flight Overview + breakdowns), old entries say so honestly, X deletes with the undo toast and never opens the sheet. |
