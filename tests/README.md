...
| `tour.test.js` | The five-step feature tour end-to-end offline: registry order (chat, roster, default, manual, menu), demo month through the real pipeline ($823.23), save beats landing in the session store while `crewAssist.archive` stays untouched, archive peek + tap-through, honest offline pivots (Default fetch, menu watcher), Manual through the real settings pill ($360.72) with the mode restored, the first-run offer shown exactly once and skippable, interruption yields + resume restarts the same step, and cleanup excises every demo bubble while the crew's own menu card stays. |
